...
- Maîtrise **Git avancé** (rebase interactif, bisect, stratégies de branching, monorepo)
- **Soft skills** techniques (code review, estimation, ADR, communication, mentorat)
- **Sécurité applicative** (OWASP Top 10, authentification, chiffrement, audit)
- **CI/CD & DevOps** (GitHub Actions, Docker en CI, déploiement, preview environments)
...
| --- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------- |
| 00  | [TypeScript](./00-typescript) | Primitives → generics → mapped types → type-level programming → variance → decorators → design patterns           | ~50h          |
| 01  | [JS Runtime](./01-js-runtime) | V8, event loop, garbage collection, JIT, hidden classes, memory leaks, profiling                                  | ~35h          |
| 05  | [Algorithms](./05-algorithms) | Complexité → structures de données → récursion → tri → arbres → graphes → backtracking → DP → patterns terrain JS | ~45h          |

**Pourquoi cet ordre ?** TypeScript pose les bases de modélisation et de sécurité de typage avant les frameworks. Les JS Runtime internals changent ensuite ta façon de penser la performance et le coût réel du code. L'algorithmie reste un socle transversal utile tout au long du parcours, sans être un prérequis immédiat pour démarrer Vue.
...
>
> **Alternative React-first** : si ta première mission est React et que tu dois démarrer vite, tu peux faire **04 → 02 → 03** (React → Vue → Angular). L'important est de maîtriser les 3 rapidement.
...
### Palier 3 — Pratiques de développement + Full-stack
...
### Phase D — Pratiques d'équipe, tester et construire le back (~235h, ~5-6 mois)

Tu sais construire 3 fronts. Maintenant tu apprends les pratiques collaboratives, tu garantis la qualite et tu construis le back-end.

| Ordre | Cours | Pourquoi maintenant | Mode |
|-------|-------|-------------------|----||
| 7 | 06 — Testing | Unit, intégration, E2E, TDD. Tu testes les 3 frameworks que tu viens d'apprendre. | Complet |
| 8 | 07 — Git Avancé | Rebase, bisect, stratégies de branching. Les workflows collaboratifs avant le travail en équipe backend. | Complet |
| 9 | 08 — Soft Skills | Code review, communication, estimation, documentation, pair programming. Les compétences d'équipe. | Complet |
| 10 | 09 — NestJS | Tu comprends enfin ce qu'il y a derriere une API. Node.js, Express, puis NestJS. | Complet |
| 11 | 10 — PostgreSQL | La couche donnees. SQL, indexes, EXPLAIN ANALYZE, puis ORM. | Complet |
...
<details>
<summary><strong>Guide d'utilisation optimale — 09 NestJS (27 modules, 26 labs)</strong></summary>
...
<details>
<summary><strong>Guide d'utilisation optimale — 10 PostgreSQL (19 modules, 18 labs, 19 quizzes)</strong></summary>
...
**Résultat tangible :** ton app fil rouge a maintenant une API NestJS, une base PostgreSQL, et des tests. C'est une vraie app fullstack. Tu maîtrises Git et les soft skills pour travailler en équipe.

**Palier de staffabilite :** après Phase D (~560h), tu es **staffable fullstack JS**.

### Phase E — Déployer et connecter (~100h, ~2 mois)

| Ordre | Cours | Pourquoi maintenant | Mode |
|-------|-------|-------------------|----||
| 12 | 11 — HTTP & Caching | Cache-Control, CDN, SSR/ISR, Redis, Service Workers. Tu comprends pourquoi ton site est lent ou rapide. | Complet |
| 13 | 12 — AWS Cloud | Tu déployés ton app fil rouge. IAM, S3, Lambda, CDK, CloudFront, CI/CD. Le module 19 (déployer Nuxt/Next) est le climax. | Complet |

<details>
<summary><strong>Guide d'utilisation optimale — 11 HTTP & Caching (21 modules, 20 labs, 21 quizzes)</strong></summary>
...
<details>
<summary><strong>Guide d'utilisation optimale — 12 AWS Cloud (20 modules, 20 labs, 20 quizzes)</strong></summary>
...
**Palier de staffabilite :** après Phase E (~660h), tu es **staffable fullstack JS avec deploy**.

### Phase F — Le palier senior (~140h, ~3 mois)

| Ordre | Cours | Pourquoi maintenant | Mode |
|-------|-------|-------------------|----||
| 14 | 13 — Architecture | SOLID, DDD, clean arch, microservices, micro-frontends. Tu as construit une vraie app fullstack — ces patterns prennent tout leur sens. | Complet |
| 15 | 14 — Sécurité Applicative | OWASP, injection, XSS, CSRF, auth, chiffrement, audit. La sécurité est une exigence senior. | Complet |
| 16 | 15 — CI/CD & DevOps | GitHub Actions, Docker en CI, déploiement, preview envs. Tu industrialises ta chaîne de livraison. | Complet |
| 17 | 16 — Observabilité & SRE | Logging, metriques, tracing, Grafana, SLOs, incidents. Tu sais construire et déployer — maintenant tu observes et maintiens. | Complet |

<details>
<summary><strong>Guide d'utilisation optimale — 13 Architecture (99 modules, 67 exercices)</strong></summary>
...
<details>
<summary><strong>Guide d'utilisation optimale — 16 Observabilité & SRE (28 modules, 26 labs, 27 quizzes)</strong></summary>
...
**Palier de staffabilite :** après Phase F (~938h), tu es **staffable senior fullstack JS** (80% des missions).

### Phase G — Elargir le spectre (~150h, ~3 mois)

| Ordre | Cours | Mode |
|-------|-------|----||
| 18 | 17 — Systèmes distribues | Partiel (~35h effectifs) : deep dive modules pratiques (00-09, 15-19), lecture modules théoriques (10-14, 20-23) |
| 19 | 18 — IA pour Devs JS | Complet. Correction : lire module 12 (Tokenization/Embeddings) avant module 09 (Transformer) |

<details>
<summary><strong>Guide d'utilisation optimale — 17 Systèmes distribues (26 modules, 24 labs)</strong></summary>
...
<details>
<summary><strong>Guide d'utilisation optimale — 18 IA pour Devs JS (20 modules, 20 labs, 20 quizzes)</strong></summary>
...
<details>
<summary><strong>Guide d'utilisation optimale — 19 React Native (28 modules, 28 labs)</strong></summary>
...
<details>
<summary><strong>Guide d'utilisation optimale — 20 WebGPU & 3D (31 modules, 30 labs)</strong></summary>
...
Phase D ─ Pratiques+Back ███████████████████████████████░░░░░░░░  ~235h  (mois 8-13)
Phase E ─ HTTP + Deploy  ███████████████████████████████░░░░░░░░  ~100h  (mois 13-15)
Phase F ─ Senior         ███████████████████████████████████░░░░  ~278h  (mois 16-21)
Phase G ─ Spectre        ████████████████████████████████████████  ~150h  (mois 22-24)
Phase H ─ Bonbons        ████████████████████████████████████████  ~115h  (optionnel)
                                                     Socle : ~1088h (~22 mois a 2h/jour)
```

**Paliers de staffabilite :**

- **Après Phase A (~195h, mois 4)** : staffable sur toute mission front-end (React, Vue/Nuxt, Angular)
- **Après Phase D (~560h, mois 13)** : staffable fullstack JS
- **Après Phase E (~660h, mois 15)** : staffable fullstack JS avec deploy
- **Après Phase F (~938h, mois 21)** : staffable senior fullstack JS (80% des missions)
- **Après Phase G (~1088h, mois 24)** : staffable sur quasi tout le spectre JS
...
Palier 3 ─ Pratiques + Full-stack ████████████████████████████████░░░░░░░  ~275h
Palier 4 ─ Archi & systemes      ██████████████████████████████████████░  ~393h
Palier 5 ─ Bonbons               ████████████████████████████████████████  ~115h
Palier 6 ─ IA                    ████████████████████████████████████████  ~60h
                                                                Total : ~1168h
```

**A 2h/jour** : ~24-28 mois | **A 3h/jour** : ~16-20 mois | **Temps plein** : ~6-7 mois
...
### 4. AWS Cloud — Nouveau cours #12

Cours complet ajoute couvrant IAM, S3, Lambda, CDK, CloudFront, DynamoDB, Cognito, déploiement Nuxt/Next. Place au Palier 4 (Architecture & Systèmes).
...
### 7. Toutes les lacunes resolues

Les lacunes senior sont couvertes par le cours Architecture (99 modules) et les 4 nouveaux cours :

- **Code review, ADR, RFC** → cours 08-Soft Skills + section `13-culture-architecturale` (Architecture)
- **Estimation & découpe de tickets** → cours 08-Soft Skills (PERT, planning poker) + exercice `60-decision-katas`
- **Migration & refactoring** → exercices `01-refactoring-solid`, `05-layered-to-hexagonal`
- **Git workflow avance** → cours 07-Git Avancé (rebase interactif, bisect, hooks, stratégies de branching, monorepo)
- **Sécurité applicative** → cours 14-Sécurité Applicative (OWASP Top 10, XSS/CSRF, auth, chiffrement, DevSecOps)
- **CI/CD & DevOps** → cours 15-CI/CD & DevOps (GitHub Actions, Docker en CI, déploiement, IaC)
...
_Curriculum généré et maintenu avec l'aide de Claude Code._
