...
> Revision rapide des fondamentaux backend du cursus.
...
| Integration | Module complet | SuperTest, TestContainers |
| E2E | Application entiere | Playwright |
...
| `@Get()` / `@Post()` / `@Put()` / `@Delete()` | Methodes HTTP |
...
|-------|-------------|-------|
| DEFAULT (singleton) | App entiere | Services stateless |
| REQUEST | Par requete | Contexte requete (tenant, user) |
| TRANSIENT | Chaque injection | Rarement necessaire |
...
- Chaque `UPDATE` cree une nouvelle version de la ligne (tuple).
...
|--------|-----------|---------------------|---------|-------|
| Read Committed (defaut) | non | oui | oui | OLTP general |
...
- Eviter `SELECT *` — projeter uniquement les colonnes necessaires.
- Utiliser `RETURNING` pour eviter un second SELECT apres INSERT/UPDATE.
...
| Query builder | Client genere, typesafe | SQL-like, typesafe |
