...
- Statut : **13/13 au format oracle (23/09) — COURS PILOTE COMPLET.** lab-09-modules : barrel type-only + export de valeur sous `verbatimModuleSyntax`, 11 tests. lab-10-utility-types : DTO Member dérivés (Omit/Pick/Partial/indexed access/Record exhaustif), 9 tests. lab-11-durcir-une-base-any (fusion des anciens 11-13) : EventBus any → generics+indexed access+mapped type interne, zéro régression sur le consommateur existant, 7 tests. lab-14-decorateurs : décorateur standard `@logged` (TS1270) + mini-DI legacy `reflect-metadata` (le mécanisme NestJS) — pipeline vitest/oxc cassé sur les décorateurs à l'exécution sur cet environnement, contourné par un run-oracle.mjs qui compile réellement avec tsc puis exécute le JS émis, 7 tests. lab-19-projet-final : le domaine TribuZen typé de bout en bout — zod + ids brandés, `Invitation` exhaustive, DTO dérivés, `Result`/`ApiClient` générique jamais un throw, 17 tests. **Le cours pilote (00-typescript) est désormais complet à 100 %** — reste dans le curriculum : 12-aws-cloud/15-cicd-devops/16-observability-sre (en pause, compte AWS requis), et 19-react-native/20-webgpu-3d/22-stripe-billing/18-ia (bibliothèque, pas encore entamés).
...
- **23 Droit** : fiches + une **checklist RGPD appliquée à TribuZen** (Z). **1/2 livré (23/09).** lab-01-checklist-rgpd-tribuzen : registre de traitements reproduisant EXACTEMENT le tableau de classification du module 01 (le vrai formulaire d'inscription TribuZen — Art.9, mineurs, minimisation) + privacy by default sur les albums photo (prouvé au niveau des types, `@ts-expect-error`). Le dev propose et remonte, ne tranche jamais seul (piège #5) — 30 tests, solution GREEN / starter RED.
