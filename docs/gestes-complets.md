...
- Statut : **1/3 livré et vérifié (23/09).** lab-01-appel-externe-resilient (Z) : `CircuitBreaker` (CLOSED/OPEN/HALF_OPEN) englobant `retryWithBackoff` (full jitter) englobant `withTimeout`, dans l'ordre exact qui évite le piège #5 du module 14 (retry au-dessus du breaker annule le breaker) — le breaker ne voit qu'un résultat final par appel, jamais les échecs absorbés par le retry. 26 tests, solution GREEN / starter RED. Reste lab-02 (saga/outbox), lab-03 (panne en cascade).
